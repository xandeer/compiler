...
enum {
  ...
  Assign, Cond, Lor, Lan, Or, Xor, And, Eq, Ne, Lt, Gt, Le, Ge, Sal, Sar, Add, Sub, Mul, Div, Mod, Inc, Dec, Brak
};
...
void next() {
  ...
  while (token = *src) {
    ...
    if (token == '\n') {
      ...
      src++;
      // if it is a single character, return Nu token
      if (token == '"') {
    ...
    while (token >= level) {
      ...
      if (token == Assign) {
        ...
        *++text = MOD;
        ...
        // on `ax` ot get its original value.
        if (*text == LI) {
          *text = PUSH;
          *++text = LI;
        } else if (*text == LC) {
...
int eval() {
  ...
  while (1) {
    ...
    else if (op == JNZ)   {pc = ax ? (int*)*pc : pc + 1;}                 // jump if ax isn't zero
...
int main(int argc, char **argv) {
  ...
  poolsize = 256 * 1025;  // arbitrary size
  line = 1;
